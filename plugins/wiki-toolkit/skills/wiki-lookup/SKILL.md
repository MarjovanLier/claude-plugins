...
1. Scan the catalogue section covering the question's domain: the destination's `index.md` or equivalent where it keeps one (`grep -n "^## " index.md` for the headings), or, in the global wiki since 2026-09-17, `scripts/catalogue.sh <subfolder>`, which prints each page's `summary` frontmatter for one topic folder; never read every folder. Treat untracked `.md` files the wiki's `git status` shows as part of the wiki.
...
Never answer from search snippets. Retrieve each shortlisted page in full and follow `related:` links and `[[wikilinks]]` one hop when a page points at a more specific one. Before asserting that a value or topic is absent, open the outbound wikilinks of every page read for the answer: that set is small, already named, and is where the contradicting value usually sits. Retrieval: `mcp__qmd__get` with `#docid` for individual results; `mcp__qmd__multi_get` only with globs or fully qualified `qmd://<collection>/<path>` values, never unqualified relative paths across collections (same-named files like `index.md` and `SCHEMA.md` collide).
...
- Absence is established only after the catalogue section, an exact `rg`, and the semantic search all come back empty and the stale-state recovery below has run; a search for a name you guessed only confirms the guess. The wiki not answering is then a valid result: say so and name the nearest pages found, and name the store that would hold the original (a mailbox for a document, a provider portal, the source repository for a running system's behaviour); search it when a tool for it is loaded, otherwise offer to, in the same reply. Wiki absence describes the index, not the world.
- A page built from an AI month summary of one chat is bounded by that file. Its "unnamed", "first name only", "the group heard nothing", or "the message stops at" is a claim about one summarised file, not about the record: before repeating it, `ls` the raw export directory for the name and grep both raw export trees for the date, and treat the raw text as outranking the page. The temporary WhatsApp qmd collections locate a described event but are never cited; their setup and the tree paths are on `pages/dev/qmd-search-setup.md`.
