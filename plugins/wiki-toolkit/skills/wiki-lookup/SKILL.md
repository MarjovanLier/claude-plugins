...
- Absence is established only after the catalogue section, an exact `rg`, and the semantic search all come back empty and the stale-state recovery below has run; a search for a name you guessed only confirms the guess. The wiki not answering is then a valid result: say so and name the nearest pages found.
