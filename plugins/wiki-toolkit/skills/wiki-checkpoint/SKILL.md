...
Set `WIKI_DIR` to the active wiki and `MEMORY_DIR` to the memory directory the harness reports for the current project. Read the destination's `SCHEMA.md` when present (otherwise sniff its layout) and, before writing, the catalogue for the folders the findings target: its index where it keeps one, or `scripts/catalogue.sh <subfolder>` in the global wiki, which prints `summary` frontmatter per page. Never impose global-wiki conventions on a project wiki.
...
- **Consumed** (write the frozen size to the watermark file, then move the snapshot out of the pending tree with `mv` to `~/.claude/wiki-checkpoint/consumed-archive-<date>/<scope>/`, refusing if the destination exists; never `rm`, and never an archive directory inside `pending/`): all valid findings persisted; or no valid durable findings existed; or the user suppressed or barred their routes. A successful local commit counts as persisted even when its push failed; the push failure is tracked by commit hash, never by keeping the transcript.
...
Before scanning any snapshot, establish the scope yourself and skip what is already done, in this order: re-list `pending/<scope>/` now, because the startup notice is a stale view of a moving set (count it with `ls <dir> | wc -l` and grep it for the session id; never print the listing, which holds every session that ever compacted and truncates the call's output; guard the glob; an empty directory is the normal case after a peer sweep); compare each file's byte size with its watermark and treat a watermark at or above the size as consumed whatever the notice says; check that the source transcript `~/.claude/projects/<slug>/<session-id>.jsonl` is not still growing (`stat -f '%Sm %z'` on both files, `ListAgents` for live peers) and retain a live session's snapshot; grep the snapshot for its own completion report (`Wiki checkpoint complete`, `Wiki ingest: complete`) and match any `Commit: <hash> pushed` it names against `git -C ~/wiki log`, so only turns after that report get a full scan. An absent watermark is not proof of unprocessed: a peer sweep that has committed but not yet disposed looks the same.
...
1. The destination's index or equivalent catalogue; in the global wiki, `scripts/catalogue.sh <subfolder>` for each topic folder the findings target.
2. Exact-pattern search for concrete terms (names, IDs, slugs, dates) over the page store.
3. qmd search through the collection matching the destination. Resolve the collection by absolute path via `mcp__qmd__status`, never by guessing its name, then `mcp__qmd__query` with lex and vec sub-queries plus an `intent` to find existing coverage. No matching collection or qmd unavailable: the fallback is a separate text search using synonyms and paraphrases of the topic. Repeating step 2's exact terms does not satisfy step 3; lexical search misses the paraphrased overlap this step exists to catch, and on any installer without qmd this fallback is the default path rather than an edge case. Two qmd shapes fail silently: a `vec` or `hyde` sub-query with a `collections` filter of exactly one entry has returned empty against a healthy index, so pass two collection names or run the vec sub-query unfiltered; and a hyphen inside a `vec` or `hyde` sub-query is parsed as negation and rejects the call, so keep hyphenated terms in the `lex` sub-query. Confirm any zero result with one `rg -n -i` over the page store before reporting absence.
...
- New pages follow the destination's conventions. Where the destination keeps an index or equivalent catalogue, add one entry per new page: one line, at most 150 characters. The global wiki keeps none since 2026-09-17: the entry is the page's own `summary` frontmatter key, double-quoted, same length rule, no trailing full stop.
...
- Never file an instruction, reassurance, figure, category boundary or timeline the source does not state. Keep the population, the clinical context and any unresolved decision as the source has them; a guideline, a laboratory protocol and a treating clinician's instruction are three sources, not one. A mechanism, magnitude or rate written from recall takes the weak citation and says what it rests on.
- Staleness applies only to `status: active` pages. A stale warning is fixed the moment it is seen, on any page, touched or not: reclassify by content (`reference` for a stable convention or a quiet case, `snapshot` for a frozen dated record, `archived` for an ended subject) or recompile against sources this machine can reach, with a scope note naming what was and was not re-verified. `last_compiled` moves only after a real recompile; never bump the date alone, and never fabricate an update. Sources this machine cannot reach decide between recompile and reclassify; they never leave the warning standing. The one page left alone is a genuinely live one whose sources are unreachable here; the report names it and the reason.
- No pages for ephemeral task detail (debugging steps, commands tried, build output).
- When wiki writes occurred and the destination keeps a log, add a newest-first entry in the destination log's own format, op `ingest` (or `lint` when only wiki-health fixes were made), ending with a one-line retrospective count summary. The global wiki keeps no log since 2026-09-17 (its git history is the operations log, see its `SCHEMA.md`), so write no entry there. Write it only after all wiki writes, including promotions, are complete. No log entry for memory-only sweeps.
...
Fix every error or warning on files the sweep touched, pre-existing or not; report issues on untouched files without fixing them, except stale-page warnings, which are fixed on sight per Step 4. No wiki writes: report `Lint: not run (no wiki found)`, `Lint: not run (skip wiki)`, or `Lint: not run (no wiki writes)`.
